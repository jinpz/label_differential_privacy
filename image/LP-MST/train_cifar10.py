...
for t in range(args.num_stage):
    save_name = f"{args.eps}_{t}_{args.num_stage}_{args.alpha}_{args.lr}"
    eps = args.eps
    ...
    trainset_t = datasets.CIFAR10(root='~/data', train=True, download=True,
                            transform=transform_train)
    ...
    if t == 0:
        if args.prior is None:
            ...
            prior_probs = torch.load(f"pretrained_priors/cifar10_{args.prior}_prior_probs.pl")[indices_t]
    ...
    cum_trainset_t = datasets.CIFAR10(root='~/data', train=True, download=True,
                            transform=transform_train)
...
#                 print(inputs.device)
    ...
    def adjust_learning_rate(optimizer, epoch):
        """grows from 0 to 0.4 in the first 15% training iterations, then it remains piecewise constant with a decay factor of 10 at the 30%, 60%, and 90% training iterations, respectively"""
        lr = args.lr
        if float(epoch)/args.epoch <= 0.15:
            lr *= (float(epoch) / args.epoch) / 0.15
        if float(epoch)/args.epoch >= 0.3:
            lr /= 10
        if float(epoch)/args.epoch >= 0.6:
            lr /= 10
        if float(epoch)/args.epoch >= 0.9:
            lr /= 10
    ...
    vanila_trainloader = torch.utils.data.DataLoader(vanila_trainset,
                                              batch_size=args.batch_size,
                                              shuffle=False, num_workers=8)
    test_loss, test_acc = test(epoch, vanila_trainloader, net=net, save=False)
    test_loss, test_acc = test(epoch, vanila_trainloader, net=best_net, save=False)
